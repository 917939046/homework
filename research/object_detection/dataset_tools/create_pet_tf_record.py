...
def dict_to_tf_example(data,
  ...
  Raises:
	...
	将XML派生的DICT转换为TF.示例PROTO。

	请注意，此函数规范提供的包围盒坐标。
	通过原始数据。

	ARG:
	数据：为单个图像保存PASCAL XML字段（由
	运行数据集
	MASKYPATH：PNG编码掩码的字符串路径。
	LabelyMaPixDITC:一个从字符串标签名到整数ID的映射。
	IMAGE子目录：指定子目录内的字符串
	Pascal DataSet目录保存实际图像数据。
	IGONORIGHARTHORTHY实例：是否跳过困难实例
	DataSet（默认值：false）。
	FaseSimION:如果是真的，为PET面生成边界框。否则
	生成包围盒（以及完整的PET体的分割）。
	...
	加薪：
	ValueError：如果数据指向的图像[文件名]不是有效的JPEG
  """
  #读取图像name
  img_path = os.path.join(image_subdirectory, data['filename'])
  #函数：读取图像 ‘r’:UTF-8编码; ‘rb’:非UTF-8编码
  ...
  #any()实现了或(OR)运算，而all()实现了与(AND)运算;axis=0表述列,axis=1表述行
